...
const val SCHEMA_VERSION_56 = 56
const val SCHEMA_VERSION_57 = 57
 ...
 * Phase AD: Apply v55→v56 changes (durable memo outbox operation identity).
...
private fun consolidateToCurrentSchema(db: SQLiteConnection) {
    ...
    // ── Phase AD: v55 → v56 (memo outbox operation identity) ──────────
    normalizeMemoFileOutboxTable(db)

    // ── Phase AE: v57 → v58 (active memo content-flag projection columns) ──
    backfillMemoContentFlagColumns(db)

    // ── Phase AF: v58 → v59 (workspace-generation-scoped sync state) ──────
    recreateWorkspaceScopedSyncStateTables(db)
