 ...
 * - Owning layer: data.
 * - Priority tier: P1.
 * - Capability: decide foreground refresh work and deferred WorkManager work from S3 endpoint profile, remote-index health, shard telemetry, trigger, and network/cadence constraints.
 *
 * Scenarios:
 * - Given auto-sync scheduling inputs, when S3 policy plans work, then fast periodic, full-reconcile periodic, and optional catch-up work are emitted as backend-neutral scheduled work.
 * - Given refresh inputs, when S3 policy plans work, then foreground scan intensity and deferred catch-up work preserve first-open, stale-shard, high-uncertainty, and strong-refresh behavior.
 ...
 *
 * Observable outcomes:
 * - foreground S3 scan payload, scheduled work cadence, unique work names, coalescing keys, network requirements, and existing-work policy.
 *
 * TDD proof:
 * - RED: `:data:testDebugUnitTest --tests com.lomo.data.sync.S3SyncWorkPolicyTest` failed to compile before the fix because S3 refresh/catch-up/reconcile policy lived behind S3-specific scheduler functions and plan DTOs instead of SyncWorkPolicy outputs.
 *
 * Excludes:
